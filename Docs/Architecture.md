...
- **`IPythonSource.TryInstallAsync(request, targetDir, context, ct)`** — return null to pass, or materialize a full install tree and return its metadata. The `SourceContext` provides shared plumbing (HTTP, checksum-verified download cache, ETag'd JSON cache) so sources stay tiny. Built-in: bundled archives (`python-embedded-runtimes/` beside the app), astral downloads. Satellite: compile-from-source (`SourceBuildSource`); custom mirrors are the obvious next one.
...
The source-build satellite (`PythonEmbedded.Net.Sources.SourceBuild`) is the third route: it resolves a version against python.org's `/ftp/python/` directory index and compiles the tarball locally. Two listings are always needed, because a pre-release lives in the directory of the final version it leads to (`/ftp/python/3.15.0/` holds `Python-3.15.0b4.tgz`), so a matching directory name proves nothing about which tarballs exist. python.org publishes no checksum manifest, so the tarball is fetched over HTTPS and hashed afterwards for provenance; `ExpectedSha256` pins it when that matters.

What makes a compiled interpreter fit into the same machinery as a downloaded one is how it is staged: `./configure --prefix=/install` plus `make install DESTDIR=<staging>/.destdir`, then the contents of `.destdir/install/` are hoisted into the staging root. That produces a tree in exactly the shape of a python-build-standalone `install_only` archive — including the literal `/install` prefix baked into `_sysconfigdata_*.py` — so the same `SysconfigPatcher` rewrites it after the host moves it into place, and nothing else in the library needs to know the interpreter was compiled. The shared `libpython` survives that move because the runpath is relative rather than patched: `$ORIGIN/../lib` on Linux, and on macOS an `@rpath` install name plus `@loader_path/../lib`, followed by an ad-hoc `codesign` (any `install_name_tool` edit invalidates the signature, and Apple silicon kills invalidly-signed binaries). Windows needs none of this — `PC\layout` emits a relocatable tree with `python.exe` at its root.
